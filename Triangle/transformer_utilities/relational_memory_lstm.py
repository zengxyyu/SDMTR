...
class RelationalMemory(nn.Module):
    ...
    def __init__(self, mem_slots, head_size, input_size, output_size, num_heads=1, num_blocks=1, forget_bias=1., input_bias=0.,
        ...
        self.attended_inp_layernorm = nn.LayerNorm( self.mem_size)
        self.attended_hx_layernorm = nn.LayerNorm( self.mem_size)
    ...
    def initial_state(self, batch_size, trainable=False):
        ...
        if False:
            ...
            init_state = torch.randn(batch_size, self.mem_size)
            return init_state
    ...
    def forward(self, inputs, memory, parallel = True):
        ...
        if not parallel:
            ...
            for i in range(self.m, inputs.size(1), self.m):
                inp = self.multihead_attention(inputs[:, :i], memory.unsqueeze(1), use_topk_ = False, store_log = False)
                inp = self.attended_inp_layernorm(inp)
                memory = self.lstm(inp.squeeze(1), memory)

            hx = self.multihead_attention(memory.unsqueeze(1), inputs, use_topk_ = False, store_log = False)
