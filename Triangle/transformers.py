...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, mask = None, num_layers = None):
        x = x.permute(1, 0, 2).to(device)  # (N257,B,D)
        x = self.pe(x)
        if not self.functional:
            ...
            for i in range(self.num_layers):
                if self.share_parameters:
                    # x, memory = self.enc(x.to(device), mask, memory = memory.to(device))
                    x, memory = self.enc(x, mask, memory=memory)  # memory每一层num_layers都不变
                else:
                    # x, memory = self.layers[i](x.to(device), mask, memory = memory.to(device))
