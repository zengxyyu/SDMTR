...
class SW_LTM(nn.Module):
    ...
    def initial_state(self, batch_size, trainable=False):
        ...
        if True:
            init_Mr = torch.stack(
                [torch.rand(self.long_mem_segs, self.mem_slots, self.mem_size) for _ in range(batch_size)])
    ...
    def multihead_attention(self, input, memory, use_topk_=True, store_log=True):
        ...
        # np.save('attention_scores_bf_top-k', scores_save)      #zxy add
        ...
        # np.save('attention_scores_after_top-k', scores_save_af)  # zxy add
...
# seq_length = 1
# batch_size = 32
# model = SW_LTM(mem_slots=10, head_size=20, input_size=input_size, num_tokens=66, num_heads=8, num_blocks=1, forget_bias=1., input_bias=0.)
