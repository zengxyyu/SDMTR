...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        #         print("多头自注意力中，初始的记忆大小为:",self.memory.size()) # (64,8,256)
        #         print("多头自注意力中，初始的Mr记忆大小为:", self.Mr.size())  # (64,8,256)
        ...
        #     _, _, self.memory, self.Mr, out_hx_mem_new = self.relational_memory(inputs=key, memory=self.memory.to(device),
        #                                                                Mr=self.Mr.to(device))
        #     # print("relational_memory返回的next_memroy大小为:{},广播更新之后的hx大小为:{}".format(self.memory.size(),out_hx_mem_new.size()))
        ...
        #     # 返回out,memory=(2,64,256),weights  memory为传入的参数,与self.memory不同
        #     return out_hx_mem_new.transpose(0, 1), memory, None
            ...
            if self.memory is None:
                self.memory = self.relational_memory.initial_state(query.size(1), query.size(0)).to(query.device)  # query(27,64,256)
                # print("使用SW原本模型！！！")  # (64,8,256)
                print("多头自注意力中，初始的记忆M大小为:", self.memory.size())  # (64,8,256)
            key = key.transpose(1, 0)  # 对key转置后为(64,27,256)
            # print(key.size())
            # memory = self.memory[:key.size(0)]
            # print(self.memory.size())
            ...
            # 包含第三步记忆更新(其为RMC主要代码)和第四步广播过程,  inputs=key为第三步的R 即[M:A]
            # key为问题和图片编码+位置编码向量 key与self.memory均为BND
            _, _, self.memory, out_hx_mem_new = self.relational_memory(inputs=key, memory=self.memory)
            # print("relational_memory返回的next_memroy大小为:{},广播更新之后的hx大小为:{}".format(self.memory.size(),out_hx_mem_new.size()))
            # print('time taken by relational:' + str(time.time() - t2))
    ...
    def init_memory(self, bs, ts = None, device = None):
        ...
        if not self.regressive:
            self.memory = self.relational_memory.initial_state(bs)
        else:
