...
class RelationalMemory(nn.Module):
    ...
    def __init__(self, mem_slots, head_size, input_size, output_size, num_heads=1, num_blocks=1, forget_bias=1., input_bias=0.,
        ...
        self.topk = topk
        self.lstm = nn.GRUCell(self.mem_size, self.mem_size)
        self.m = mem_slots
    ...
    def initial_state(self, batch_size, trainable=False):
        ...
        if False:
            ...
            init_state = torch.randn(batch_size, 1, self.mem_size)
            return init_state

    def multihead_attention(self, input, memory, use_topk_ = True, store_log = True, normalize = True):
        """
    ...
    def forward(self, inputs, memory, parallel = True):
        ...
        if not parallel:
            ...
            for i in range(self.m, inputs.size(1), self.m):
                # inp为inputs与memory注意力的结果，为新的记忆
                inp = self.multihead_attention(inputs[:, :i], memory, use_topk_ = False, store_log = False, normalize=False)
                inp = self.attended_memory_layernorm(inp)
                B, S, D = memory.size()
                inp = inp.reshape(inp.size(0) * inp.size(1), -1)
                memory = memory.reshape(memory.size(0) * memory.size(1), -1)
                # 新记忆与原来的记忆经过LSTM处理
                memory = self.lstm(inp, memory)
                memory = memory.reshape(B, S, -1)
                #new_memory = self.multihead_attention(memory, memory, use_topk_ = False, store_log = False, normalize = False)
                #memory = memory + new_memory
            # q = inputs k,v = memory
            # 此处的hx为第四步广播地方的更新的hx
            hx = self.multihead_attention(memory, inputs, use_topk_ = False, store_log = False, normalize = False)
